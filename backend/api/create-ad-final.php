<?php
/**
 * Create Ad API - FINAL VERSION (after database fix)
 * POST /api/create-ad.php
 * Creates a new marketplace ad (requires authentication)
 * This version expects all columns to exist after running fix-marketplace-ads-columns.php
 */
...
// Insert ad with ALL columns (after migration)
$stmt = $con->prepare("
    INSERT INTO marketplace_ads
    (user_id, title, description, price, category, subcategory, `condition`, location, address, seller_name, seller_phone, seller_email, specifications, status, featured, views)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', 0, 0)
");
