...
const Swipeable = props => {
  ...
  const dispatchImmediateEvents = (0, _react.useCallback)((fromValue, toValue) => {
    ...
    if (onSwipeableWillOpen && toValue !== 0) {
      (0, _reactNativeReanimated.runOnJS)(onSwipeableWillOpen)(toValue > 0 ? _ReanimatedSwipeableProps.SwipeDirection.RIGHT : _ReanimatedSwipeableProps.SwipeDirection.LEFT);
    }
    if (onSwipeableWillClose && toValue === 0) {
      (0, _reactNativeReanimated.runOnJS)(onSwipeableWillClose)(fromValue > 0 ? _ReanimatedSwipeableProps.SwipeDirection.LEFT : _ReanimatedSwipeableProps.SwipeDirection.RIGHT);
    }
  ...
  const dispatchEndEvents = (0, _react.useCallback)((fromValue, toValue) => {
    ...
    if (onSwipeableOpen && toValue !== 0) {
      (0, _reactNativeReanimated.runOnJS)(onSwipeableOpen)(toValue > 0 ? _ReanimatedSwipeableProps.SwipeDirection.RIGHT : _ReanimatedSwipeableProps.SwipeDirection.LEFT);
    }
    if (onSwipeableClose && toValue === 0) {
      (0, _reactNativeReanimated.runOnJS)(onSwipeableClose)(fromValue > 0 ? _ReanimatedSwipeableProps.SwipeDirection.LEFT : _ReanimatedSwipeableProps.SwipeDirection.RIGHT);
    }
  ...
  const panGesture = (0, _react.useMemo)(() => {
    const pan = _.Gesture.Pan().enabled(enabled !== false).enableTrackpadTwoFingerGesture(enableTrackpadTwoFingerGesture).activeOffsetX([-dragOffsetFromRightEdge, dragOffsetFromLeftEdge]).onStart(updateElementWidths).onUpdate(event => {
      userDrag.value = event.translationX;
      const direction = rowState.value === -1 ? _ReanimatedSwipeableProps.SwipeDirection.RIGHT : rowState.value === 1 ? _ReanimatedSwipeableProps.SwipeDirection.LEFT : event.translationX > 0 ? _ReanimatedSwipeableProps.SwipeDirection.RIGHT : _ReanimatedSwipeableProps.SwipeDirection.LEFT;
